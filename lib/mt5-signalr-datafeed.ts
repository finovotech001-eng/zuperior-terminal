...
export class MT5SignalRDatafeed {
  private hubUrl = 'http://18.175.242.21:5003/hubs/chart'
  ...
  private async ensureConnection() {
    ...
    try {
      ...
      const { HubConnectionBuilder, LogLevel } = signalR as any
      ...
      this.connected = true
      console.log('✅ Chart SignalR connected')
    } catch (error) {
      console.error('❌ Chart SignalR connection failed:', error)
      throw error
    }
  }
